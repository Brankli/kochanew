...
onMounted(async () => {
    await axios.get('./api/getall/sponser').then(res => {
...
const searching = computed(() => {
    ...
    return sponsers.value.filter(item => item.CompanyName.toLowerCase().includes(keyword));
});
    ...
    <div class="bg-gray-100">
        ...
        <div class="m-4 mt-12 rounded">
            <div class="w-full bg-white border-b-2 rounded border-gray-200 flex flex-row justify-between">
                <h1 class="font-bold text-gray-500 text-lg p-4 ">Sponsers List</h1>
                <router-link to="/newsponsers"
            ...
            <div class="flex bg-white flex-col items-center justify-center">
                <div class=" w-full  relative p-2 overflow-x-auto shadow-md sm:rounded-lg xs:p-5">
                    ...
                    <table class="w-full text-sm text-left rtl:text-right text-gray-500">
                        ...
                        <tbody v-for="sponser in paginatedData" :key="sponser.id">
                            <tr class="bg-white border-b hover:bg-gray-50">
                                <td @click.prevent="countTrashSponsers(sponser.id)" class="w-4 p-4">
                                    <div class="border-4 border-gray-200 text-center rounded w-5 h-5">
                                        <p v-if="removedSponsolId.includes(sponser.id)"><i
                                ...
                                <td class="px-6 py-4">
                                    <div class="w-fit flex flex-col md:mt-0 hover:ml-4 m-3 xl:p-0">
                                        <div class="flex gap-3 ">
                                            <img :src="sponser.image" class="w-16 h-10 object-cover" alt="work-image"> <p
                                                class="hover:text-sky-800">{{ sponser.CompanyName }}</p>
                                        </div>
                                    </div>
                                </td>
                            </tr>
