...
onMounted(async () => {
    await axios.get('./api/getall/testimonial').then(res => {
...
const hidenSponsor = () => {
    ...
    axios.post('./api/testimonial/hidden', { id: removedSponsolId.value }).then(res => {
...
const searching = computed(() => {
    ...
    return testimonial.value.filter(item => item.Name.toLowerCase().includes(keyword));
});
                    ...
                    <table class="w-full text-sm text-left rtl:text-right text-gray-500">
                        ...
                        <tbody v-for="testimonial in paginatedData" :key="testimonial.id">
                            <tr class="bg-white border-b hover:bg-gray-50">
                                <td @click.prevent="countTrashSponsers(testimonial.id)" class="w-4 p-4">
                                    <div class="border-4 border-gray-200 text-center rounded w-5 h-5">
                                        <p v-if="removedSponsolId.includes(testimonial.id)"><i
                                ...
                                <td class="px-6 py-4">
                                    <div class="w-fit flex flex-col md:mt-0 hover:ml-4 m-3 xl:p-0">
                                        <div class="flex gap-3 ">
                                            <img :src="testimonial.image" class="w-16 h-10 object-cover" alt="work-image">
                                        </div>
                                    </div>
                                </td>
                                <td>
                                ...
                                <td>
                                    <p class="hover:text-sky-800">{{ testimonial.discribution }}</p>
